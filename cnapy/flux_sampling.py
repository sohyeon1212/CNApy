...
def compute_sampling_statistics(samples: pd.DataFrame) -> tuple[pd.Series, pd.Series, pd.DataFrame]:
    """
...
def add_gaussian_noise_to_samples(
    samples: pd.DataFrame, reference_fluxes: dict[str, float], std_fraction: float = 0.1
) -> pd.DataFrame:
    """
