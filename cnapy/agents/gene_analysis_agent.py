...
class GeneAnalysisAgent(BaseAgent):
    ...
    def _knockout_gene(self, gene_id: str) -> SkillResult:
        ...
        try:
            ...
            affected_rxn_info = [{"id": r.id, "name": r.name} for r in affected_reactions]
    ...
    def _find_essential_reactions(self, threshold: float = 0.01) -> SkillResult:
        ...
        try:
            ...
            return SkillResult(
                ...
                message=(f"Found {len(essential_reactions)} essential reactions out of {len(model.reactions)} total."),
                message_ko=(f"총 {len(model.reactions)}개 반응 중 {len(essential_reactions)}개 필수 반응 발견."),
            )
    ...
    def _get_gene_info(self, gene_id: str) -> SkillResult:
        ...
        reactions = [{"id": r.id, "name": r.name, "gpr": str(r.gene_reaction_rule)} for r in gene.reactions]
        ...
        return SkillResult(
            ...
            message=(f"Gene {gene.id}: Associated with {len(reactions)} reaction(s)."),
            message_ko=(f"유전자 {gene.id}: {len(reactions)}개 반응과 연관됨."),
        )
    ...
    def _analyze_gene_impact(self, gene_id: str) -> SkillResult:
        ...
        impact_level = "High" if ko_data["is_essential"] else ("Medium" if ko_data["growth_ratio"] < 0.5 else "Low")
