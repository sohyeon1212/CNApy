...
from qtpy.QtWidgets import (
    ...
    QSpinBox, QProgressBar, QTabWidget, QWidget, QSplitter,
...
class DynamicFBADialog(QDialog):
    ...
    def __init__(self, appdata: AppData, parent=None):
        ...
        self.simulation_thread: Optional[DFBASimulationThread] = None
        self.last_result: Optional[DFBAResult] = None
    ...
    def _setup_ui(self):
        ...
        self.products_table.setHorizontalHeaderLabels([
            "Include", "Exchange Reaction", "Initial (mM)"
        ])
    ...
    def _run_simulation(self):
        ...
        self._log(f"Starting dFBA simulation...")
    ...
    def _on_simulation_finished(self, result: DFBAResult):
        ...
        if result.success:
            ...
            QMessageBox.information(self, "Simulation Complete", 
                ...
                f"Final biomass: {result.biomass[-1]:.4f} g/L")
        else:
    ...
    def _update_plot(self, result: DFBAResult):
        ...
        for i, (mid, conc) in enumerate(result.metabolites.items()):
            ax2.plot(result.time, conc, color=colors[i % len(colors)], 
                    linewidth=1.5, label=mid)
    ...
    def _update_data_table(self, result: DFBAResult):
        ...
        for row in range(n_rows):
            ...
            for col, mid in enumerate(result.metabolites.keys()):
                self.data_table.setItem(row, 2 + col, 
                    QTableWidgetItem(f"{result.metabolites[mid][row]:.6f}"))
