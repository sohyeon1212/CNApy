...
from qtpy.QtWidgets import QDialog, QFrame, QGroupBox, QHBoxLayout, QLabel, QPushButton, QVBoxLayout
from straindesign import avail_solvers, linexpr2dict, yopt
...
class YieldSpaceDialog(QDialog):
    ...
    def __init__(self, appdata):
        ...
        text = QLabel(
            "Specify the yield terms that should be used for the horizontal and yical axis.\n"
        ...
        self.x_numerator = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True
        )
        self.x_numerator.setPlaceholderText("numerator (e.g. 1.0 " + r1 + ")")
        self.x_denominator = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True
        )
        ...
        self.y_numerator = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True
        )
        self.y_numerator.setPlaceholderText("numerator (e.g. " + r3 + ")")
        self.y_denominator = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True
        )
    ...
    def compute(self):
        ...
        with self.appdata.project.cobra_py_model as model:
            ...
            if any(unbnd):
                raise Exception(
                    "One of the specified yields is unbounded or undefined. Yield space cannot be generated."
                )
            # compute points
            points = 50
            ...
            for i in range(points):
                ...
                sol_vmin = yopt(
                    model, constraints=constr, obj_num=y_num, obj_den=y_den, solver=solver, obj_sense="minimize"
                )
                lb[i] = sol_vmin.objective_value
                sol_vmax = yopt(
                    model, constraints=constr, obj_num=y_num, obj_den=y_den, solver=solver, obj_sense="maximize"
                )
