...
from qtpy.QtWidgets import QComboBox, QDialog, QFrame, QHBoxLayout, QLabel, QMessageBox, QPushButton, QVBoxLayout
...
try:
    from straindesign import avail_solvers, linexpr2dict, linexprdict2str, yopt
...
class YieldOptimizationDialog(QDialog):
    ...
    def __init__(self, appdata: AppData, central_widget: CentralWidget):
        ...
        l = QLabel(
            "Maximize (or minimize) a yield function. \n"
        ...
        self.numerator = QComplReceivLineEdit(
        ...
        self.denominator = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True
        )
        ...
        with self.appdata.project.cobra_py_model as model:
            ...
            if sol.status == UNBOUNDED and isinf(sol.objective_value):
                ...
                QMessageBox.information(
                    ...
                    + "\nShowing yield-optimal example flux distribution."
                    + txt_scalable,
                )
            else:
                QMessageBox.warning(
    ...
    def set_boxes(self, sol):
        ...
        for r in self.reac_ids:
            self.appdata.project.comp_values[r] = (float(sol.fluxes[r]), float(sol.fluxes[r]))
            idx = idx + 1
