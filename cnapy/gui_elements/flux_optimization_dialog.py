...
from qtpy.QtWidgets import QComboBox, QDialog, QHBoxLayout, QLabel, QMessageBox, QPushButton, QVBoxLayout
...
class FluxOptimizationDialog(QDialog):
    ...
    def __init__(self, appdata: AppData, central_widget: CentralWidget):
        ...
        l = QLabel(
            "Maximize (or minimize) a linear flux expression with reaction identifiers and \n"
    ...
    def compute(self):
        ...
        with self.appdata.project.cobra_py_model as model:
            ...
            if solver is not None:
                solver = solver[0]
            sol = fba(model, obj=self.expr.text(), obj_sense=self.sense_combo.currentText())
            if sol.status == UNBOUNDED and isinf(sol.objective_value):
                ...
                QMessageBox.warning(
                    ...
                    sense + " unbounded. ",
                    'Flux expression "'
                    + linexprdict2str(linexpr2dict(self.expr.text(), self.reac_ids))
