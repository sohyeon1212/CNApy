...
from cnapy.utils import QComplReceivLineEdit, QTableCopyable, QTableItem, show_unknown_error_box
...
class SDDialog(QDialog):
    ...
    def __init__(self, appdata: AppData, sd_setup: dict = {}):
        ...
        if (
            not hasattr(self.appdata.project.cobra_py_model, "genes")
            or len(self.appdata.project.cobra_py_model.genes) == 0
        ):
            self.gene_wordlist = self.reac_wordlist
            self.gene_ids = []
            self.gene_names = []
        else:
            self.gene_wordlist = set(
        ...
        self.module_edit[OUTER_OBJECTIVE] = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode
        )
        self.module_edit[OUTER_OBJECTIVE].setPlaceholderText(
            "e.g., EX_succ_e or EX_lac__D_e (exchange reaction of target product)"
        )
        ...
        self.module_edit[INNER_OBJECTIVE + "_label"] = QLabel(
            "Inner objective (maximized) - Cell's objective (typically biomass)"
        )
        self.module_edit[INNER_OBJECTIVE + "_label"].setHidden(True)
        self.module_edit[INNER_OBJECTIVE] = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode
        )
        self.module_edit[INNER_OBJECTIVE].setPlaceholderText(
            "e.g., BIOMASS_Ec_iJO1366_core_53p95M or Biomass_Ecoli_core"
        )
        ...
        self.module_edit[PROD_ID] = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode
        )
        ...
        self.use_scenario = QCheckBox(" Use scenario")
        ...
        for i, r in enumerate(self.reac_ids):
            ...
            self.reaction_itv.update({r: {"cost": QTableItem("1.0"), "button_group": QButtonGroup()}})
            ...
            self.reaction_itv[r]["button_group"].buttonClicked.connect(lambda state, x=r: self.knock_changed(x, "reac"))
        # buttons
        ...
        for i, g in enumerate(self.gene_ids):
            ...
            self.gene_itv.update({g: {"cost": QTableItem("1.0"), "button_group": QButtonGroup()}})
            ...
            self.gene_itv[g]["button_group"].buttonClicked.connect(lambda state, x=g: self.knock_changed(x, "gene"))
        # buttons
        self.all_koable = QPushButton("All targetable")
    ...
    def add_constr(self):
        ...
        constr_entry = QComplReceivLineEdit(
            self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True, is_constr=True
        )
        constr_entry.setPlaceholderText(self.placeholder_eq)
        self.active_receiver = constr_entry
    ...
    def add_reg(self):
        ...
        reg_entry = QComplReceivLineEdit(
            self, self.gene_wordlist, self.appdata.is_in_dark_mode, check=True, is_constr=True
        )
    ...
    def update_module_edit(self):
        ...
        self.module_spec_box.setTitle(
            "Module " + str(self.current_module + 1) + " specifications (" + module_type + ")"
        )
        if not self.modules[self.current_module]:
            ...
            for i, c in enumerate(mod[CONSTRAINTS]):
                ...
                self.module_edit[CONSTRAINTS].insertRow(i)
                constr_entry[i] = QComplReceivLineEdit(
                    self, self.appdata.project.reaction_ids, self.appdata.is_in_dark_mode, check=True, is_constr=True
                )
            ...
            if mod[INNER_OBJECTIVE]:
                ...
                )  # add space character to avoid
                self.module_edit[INNER_OBJECTIVE].check_text(True)  # word completion
            if mod[OUTER_OBJECTIVE]:
                self.module_edit[OUTER_OBJECTIVE].setText(linexprdict2str(mod[OUTER_OBJECTIVE]) + " ")
                self.module_edit[OUTER_OBJECTIVE].check_text(True)
            if mod[PROD_ID]:
                self.module_edit[PROD_ID].setText(linexprdict2str(mod[PROD_ID]) + " ")
                self.module_edit[PROD_ID].check_text(True)
            if mod[MIN_GCP]:
    ...
    def verify_module(self, *args):
        ...
        constraints = [
            self.module_edit[CONSTRAINTS].cellWidget(i, 0).text()
        ...
        if module_type in [PROTECT_STR, SUPPRESS_STR] and (
            not self.module_edit[NESTED_OPT].isChecked() or not self.module_edit[INNER_OBJECTIVE]
        ):
    ...
    def update_global_objective(self, b=True):
        ...
        if not modules:
            ...
            self.global_objective.setText(
                self.global_objective.property("prefix")
            ...
            confl_modules = [
                (i + 1, m) for i, m in enumerate(modules) if m in [OPTKNOCK_STR, ROBUSTKNOCK_STR, OPTCOUPLE_STR]
            ]
            self.global_objective.setStyleSheet(FONT_COLOR("#de332a"))
            self.global_objective.setText(
                "Conflicting modules: " + ", ".join([str(m[0]) + " (" + m[1] + ")" for m in confl_modules])
            )
        elif sum([1 for m in modules if m in [OPTKNOCK_STR, ROBUSTKNOCK_STR, OPTCOUPLE_STR]]) == 1:
            main_module = [
                (self.modules[i], i, m)
                for i, m in enumerate(modules)
                if m in [OPTKNOCK_STR, ROBUSTKNOCK_STR, OPTCOUPLE_STR]
            ][0]
            ...
            if main_module[0] is not None and main_module[2] == OPTCOUPLE_STR:
                ...
                if objective and prod_id:
                    self.global_objective.setText(
                        self.global_objective.property("prefix")
                        + "maximize coupling potential for "
                        + prod_id
                        + ", s.t.: max{"
                        + objective
                        + "} (module "
            ...
            if main_module[1] == self.current_module:
                try:
                    if main_module[2] == OPTCOUPLE_STR:
                        objective = linexprdict2str(
                            linexpr2dict(self.module_edit[INNER_OBJECTIVE].text(), self.reac_wordlist)
                        )
                        ...
                        objective = linexprdict2str(
                            linexpr2dict(self.module_edit[OUTER_OBJECTIVE].text(), self.reac_wordlist)
                        )
                        if objective:
                            self.global_objective.setText(
                                self.global_objective.property("prefix")
                    ...
                    self.global_objective.setStyleSheet(FONT_COLOR("#000000"))
                    self.global_objective.setText(
                        "Editing module ... (module " + str(main_module[1] + 1) + ": " + main_module[2] + ")"
                    )
                    return
            self.global_objective.setStyleSheet(FONT_COLOR("#de332a"))
            self.global_objective.setText(
                self.global_objective.property("prefix")
                + "Global/Outer objective invalid or missing (module "
                + str(main_module[1] + 1)
                + ": "
                + main_module[2]
                + ")"
    ...
    def ko_ki_filter_text_changed(self):
        ...
        hide_genes = [
            True if txt not in g.lower() and txt not in n.lower() else False
            for g, n in zip(self.gene_ids, self.gene_names, strict=False)
        ]
    ...
    def knock_changed(self, id, gene_or_reac):
        if gene_or_reac == "reac":
            ...
            if checked in [1, 3]:
                ...
                for g, gid in zip([self.gene_itv[s] for s in genes], genes, strict=False):
            ...
            if checked in [1, 3]:
                ...
                g["cost"].setText("")
                g["cost"].setEnabled(False)
                for r, rid in zip([self.reaction_itv[s] for s in reacs], reacs, strict=False):
                    # reactivate only if all genes of this reaction have been deactivated
    ...
    def parse_dialog_inputs(self):
        ...
        if sd_setup["advanced"]:
            ...
            kiCost = {}
            regCost = {
                self.regulatory_itv_list.cellWidget(i, 0).text(): float(self.regulatory_itv_list.item(i, 1).text())
    ...
    def save(self):
        ...
        filename: str = dialog.getSaveFileName(directory=self.appdata.work_directory, filter="*.sdc")[0]
    ...
    def load(self, sd_setup={}):
        if sd_setup == {} or sd_setup == False:
            # open file dialog
            dialog = QFileDialog(self)
            filename: str = dialog.getOpenFileName(directory=self.appdata.last_scen_directory, filter="*.sdc")[0]
        ...
        if sd_setup["advanced"]:
            ...
            if REGCOST in sd_setup:
                ...
                for i, (k, v) in enumerate(sd_setup[REGCOST].items()):
                    self.regulatory_itv_list.insertRow(i)
                    reg_entry[i] = QComplReceivLineEdit(
                        self, self.gene_wordlist, self.appdata.is_in_dark_mode, check=True, is_constr=True
                    )
    ...
    def compute(self):
        ...
        if not self.modules:
            QMessageBox.information(
                self, "Please add modules", "At least one module must be added to the " + "strain design problem."
            )
            return
        if any([True for m in self.modules if m is None]):
            QMessageBox.information(
                self,
                "Please complete module setup",
        ...
        if self.solver_buttons["OPTLANG"].isChecked():
            ...
            if len(sd_setup.get(REGCOST, [])) > 0:
                if (
                    QMessageBox.information(
                        self,
                        "Regulatory interventions not supported",
            ...
            if any(m[INNER_OBJECTIVE] is not None for m in sd_setup[MODULES]):
                if (
                    QMessageBox.information(
                        self,
                        "Inner objectives not supported",
        ...
        with self.appdata.project.cobra_py_model as model:
            ...
            try:
                mcs, err_val = mcs_computation.compute_mcs(
                    ...
                    timeout=timeout,
                    cuts=cuts,
                    knock_in_idx=knock_in_idx,
                    intervention_costs=iv_costs,
...
class SDViewer(QDialog):
    ...
    def __init__(self, appdata: AppData, solutions, with_setup: bool):
        ...
        if self.solutions.is_gene_sd:
            ...
            self.sd_table.setHorizontalHeaderLabels(
                ["Equiv. class", "Intervention set", "Reaction-phenotype interventions"]
            )
            ...
            for i, a, g in zip(range(len(self.gsd)), self.assoc, self.gsd, strict=False):
        ...
        if self.solutions.has_complex_regul_itv:
            QMessageBox.information(
                self,
                "Non-trivial regulatory interventions",
    ...
    def savesdtsv(self):
        # open file dialog
        dialog = QFileDialog(self)
        filename: str = dialog.getSaveFileName(directory=self.appdata.work_directory, filter="*.tsv")[0]
        if not filename or len(filename) == 0:
            return
        ...
        if self.solutions.is_gene_sd:
            sd_string = "\n".join(
                ["\t".join([str(a), g, self.rsd[a]]) for a, g in zip(self.assoc, self.gsd, strict=False)]
            )
        else:
            sd_string = "\n".join(self.rsd)
