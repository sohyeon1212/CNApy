...
class FluxVectorContainer:
    def __init__(self, matORfname, reac_id=None, irreversible=None, unbounded=None):
        if type(matORfname) is str:
            try:
                l = numpy.load(
                    matORfname, allow_pickle=True
                )  # allow_pickle to read back sparse matrices saved as fv_mat
    ...
    def __getitem__(self, idx):
        return {
            self.reac_id[i]: float(self.fv_mat[idx, i]) for i in range(len(self.reac_id)) if self.fv_mat[idx, i] != 0
        }

    def save(self, fname):
        numpy.savez_compressed(
            fname, fv_mat=self.fv_mat, reac_id=self.reac_id, irreversible=self.irreversible, unbounded=self.unbounded
        )
...
class FluxVectorMemmap(FluxVectorContainer):
    ...
    def __init__(self, fname, reac_id, containing_temp_dir=None):
        ...
        super().__init__(
            numpy.memmap(self._memmap_fname, mode="r+", dtype=">d", offset=13, shape=(num_efm, num_reac), order="C"),
            reac_id,
        )
